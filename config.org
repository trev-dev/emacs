...
(add-to-list 'td/package-list 'mood-line)
(mood-line-mode)
#+end_src
...
Because it has its own keymap, I can add utility functions to god-mode. This turns it into sort of a pseud-modal editing mode. However, unlike other modal packages, it does not require as much key re-binding, thanks to key-chord translation.
...
Declare key-bindings, apply them to =god-local-mode-map= and give the keymap priority by adding it to the =emulation-mode-map-alists=.
