...
(defun td/lsp-mode-config ()
  ...
  (td/bind-many-keys '(("C-c l c d" . consult-lsp-diagnostics)
...
2. Optionally checkout an alternate edition. Your branch choices are:
   ...
   - *holy* - A pure approach to emacs using only supported internals wherever possible. This is my best attempt at "minimalism" with Emacs.
...
The *holy* branch is an experimental config geared toward staunch support for using supported internals only. It uses packages like Eglot, Corfu, Cape, etc.
