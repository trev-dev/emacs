...
(defvar td/todo-keywords
  '((sequence "TODO(t)" "NEXT(n)" "WAIT(w@/!)" "|" "DONE(d!)" "PASS(p@)" "CANC(k@)"))
  "A sequence of keywords for Org headlines.")
#+END_SRC

My org agenda commands. Currently a work in progress.
...
We use =td/org-scale-levels= to improve the readability of org-mode headers in order of the highest level sections to as low as 3 levels. For myself, anything deeper than 3 levels is just bold text.

Later on, =td/org-hook= will be fired when org-mode is started and it contains complimentary settings/modes.
...
- [[https://github.com/Chobbes/org-chef][org-chef]] is a must have if you enjoy cooking
- ox-gfm is for Github Flavored Markdown
- visual-fill-column is for narrowing org-mode into a reading-friendly column size.
- org-alert lets me get notifications regarding agenda items.
