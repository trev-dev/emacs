...
(td/add-setup 'tree-sitter
  (with-eval-after-load 'tree-sitter-langs
    ;; Tree sitter doesn't play well with html mode. Set up a special setup
    ;; method for activating tree-sitter on all supported langs except those
    ;; listed as excluded.
    (let ((excluded-modes '(html-mode))
          (ts-modes (mapcar
