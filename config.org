...
*** Bootstrapping
Declare repositories, init package.el...but don't enable packages at startup.
...
(setq package-enable-at-startup nil) ; Please don't start loading packages right away.
(package-initialize)
#+end_src
...
(unless (package-installed-p 'quelpa)
  (with-temp-buffer
    ...
    (eval-buffer)
    (quelpa-self-upgrade)))
(setq quelpa-update-melpa-p nil) ; Please don't attempt to replace package.el for melpa.
(require 'quelpa)
#+end_src
...
#+begin_src elisp :tangle yes
(defvar td/package-list '(quelpa) "My explicit list of packages from melpa")
...
#+begin_src elisp :tangle yes
(defun td/bind-many (conses &optional mode-map)
...
(defun td/consult-config ()
  ...
  (td/bind-many '(("C-c h" . consult-history)
  ...
  ;; Consult completion-at-point solution.
...
#+begin_src elisp :tangle yes
(add-to-list 'td/package-list 'savehist)
(add-to-list #'td/package-configs #'(lambda () "Savehist config."
                                      (savehist-mode)))
#+end_src
...
(add-to-list 'td/package-configs #'global-undo-tree-mode)
#+end_src
...
(add-to-list 'td/package-list 'eldoc-box)
...
(package-autoremove)
(dolist (conf td/package-configs)
  (funcall conf))
#+end_src
** About This Config
