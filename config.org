...
(defun tdm/vc ()
  ...
  (let* ((vc vc-mode)
        ...
        (icon (if dirty "±" "✔")))
    (format "%s" (propertize
                      (substring vc 5) 'face (if (tdm/git-dirty?)
                                                 'warning
                                               'success)))))
...
(defun tdm/project()
  ...
  (when (project-current)
    (concat (propertize (if (stringp td/custom-project-name)
            ...
            (propertize ":" 'face
                        '(:inherit font-lock-comment-face :background nil)))))
...
(defvar tdm/buffer-position
  '("  %l:%c"
    (:propertize " %p" face (:inherit font-lock-comment-face :background nil))))
...
(td/add-setup 'god-mode
  ...
  (define-key isearch-mode-map (kbd "s-g") #'god-mode-isearch-activate)
  (define-key god-mode-isearch-map (kbd "s-g") #'god-mode-isearch-disable)
