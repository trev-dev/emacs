...
- [[#package-configuration][Package Configuration]]
  - [[#bootstrapping][Bootstrapping]]
    ...
    - [[#maintain-selected-packages][Maintain selected packages]]
    - [[#quelpa][Quelpa]]
  - [[#look--feel][Look & Feel]]
...
*** Bootstrapping
I am using Quelpa for strange repositories that aren't in a package.el friendly space. I am otherwise configuring packages with use-package.
...
**** Maintain selected packages
I want to be able to properly maintain my selected packages so that ~package-autoremove~ works as a maintenance function that I can run to remove forgotten packages I had tried out, or stopped using in this init.

#+BEGIN_SRC elisp
(defvar td/package-list '(quelpa))

(defun td/select-package (pkg)
  "Install, and add `PKG' to `td/package-list'."
...
#+BEGIN_SRC elisp
(td/select-package 'all-the-icons)
...
#+BEGIN_SRC elisp
(td/select-package 'all-the-icons-dired)
...
#+BEGIN_SRC elisp
(td/select-package 'telephone-line)
...
(td/select-package 'consult-flycheck)
...
#+BEGIN_SRC elisp
(td/select-package 'elfeed)

(use-package elfeed
  :bind ("M-o e" . elfeed))

(td/select-package 'elfeed-org)
...
(td/select-package 'plz)
(td/select-package 'ement)
...
#+BEGIN_SRC elisp
(td/select-package 'expand-region)
...
#+BEGIN_SRC elisp
(td/select-package 'exec-path-from-shell)
...
(td/select-package 'org-make-toc)
...
(td/select-package 'visual-fill-column)
...
(td/select-package 'flycheck-ledger)
...
#+BEGIN_SRC elisp
(dolist (pkg '(lsp-mode lsp-ui consult-lsp))
  (td/select-package pkg))
...
#+BEGIN_SRC elisp
(td/select-package 'markdown-mode)
...
#+BEGIN_SRC elisp
(td/select-package 'rainbow-delimiters)
...
#+BEGIN_SRC elisp
(td/select-package 'rainbow-mode)
...
#+BEGIN_SRC elisp
(td/select-package 'tree-sitter)

(use-package tree-sitter
  :hook (tree-sitter-after-on . tree-sitter-hl-mode)
  :config (global-tree-sitter-mode))

(td/select-package 'tree-sitter-langs)
...
#+BEGIN_SRC elisp
(dolist (pkg '(typescript-mode js2-mode prettier-js))
  (td/select-package pkg))
...
#+BEGIN_SRC elisp
(dolist (pkg '(yasnippet yasnippet-snippets)) (td/select-package pkg))
