* Emacs Configuration - Saintly Edition
:PROPERTIES:
#+STARTUP: overview
:END:
Herein lies my *Saintly* Emacs configuration. Use it well
** Contents
:PROPERTIES:
...
2. (Optional) Checkout a flavor branch. Your options are currently:
   - Evil Branches
     - =main= - My current working setup with evil bindings.
     - =org-mode= - A slim config for using org-mode. This is intended as a simple branch for documenting, organizing my agenda & ideas as well as writing email. It looks a lot like =main= but without any of the programming stuff. In my opinion, org-mode is the only irreplacable feature of Emacs. This config is for keeping Emacs light and useful for what it does best for me, should I ever stray from Emacs for writing code. This branch is also evil.
   - Holy Branches
     - =emacs-keybinds= - Follows the standard config - but uses hydra instead of evil-mode to try and make things a bit more ergonomic.
     - =saintly= - An experimental configuration that uses standard bindings & supported internals wherever possible. It's filled with alternative packages like Eglot, Cape/Corfu/Vertico, etc.
