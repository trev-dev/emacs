...
(require 'ido)
(require 'icomplete)
(setq ido-enable-flex-matching t) ; Use flex matching to help find things more easily
(fido-mode 1) ; Use "fake ido" for completions other than M-x
#+end_src
...
*** God Mode
Some of the native keybinds are hard for me to wrap my head around. Take ~C-M-@~, for instance.

All of these keys required to do this command are on the left-hand-side of the keyboard. For myself, the compulsion is to try and do hit them all with my left hand.

I'm getting used to using my right hand for Meta for many things, but it's taking time. My previous experiences with Vim were more comfortable/simple to adapt to than learning ambidexterity.

I don't want to re-map all of the things in Emacs. For now, what I will do, is reduce the number of simultaneous button presses with God Mode.

#+begin_src emacs-lisp
(defun td/god-mode-hook ()
  "Make a modeline agnostic way for me to know I'm in God Mode"
  (if god-local-mode
      (progn
        (setq prev-cursor (cdr (assoc 'cursor-type (frame-parameters))))
        (setq cursor-type 'hollow))
    (setq cursor-type prev-cursor)))

(use-package god-mode
  :ensure t
  :hook (post-command . td/god-mode-hook)
